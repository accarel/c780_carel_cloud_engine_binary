cd ~/esp
# this will create a folder esp-idf with version 4.3.1
...
patch components/freemodbus/common/esp_modbus_master.c ~/esp/GME_Binary/patches/0001_esp_modbus.patch

# manage cmd 17
cd components/freemodbus/modbus
patch -p0 < ~/esp/GME_Binary/patches/0002_manage_cmd17
  
#   MANCANO QUESTE DUE!!! da unire in una sola
#  reset fifo queue uart (ex collect messages longer than 120 bytes...patched from esp-idf 4.3.1) 
cd components/freemodbus
  patch -p0 < ~/esp/GME_Binary/patches/0003-clear_uart_ex_120.patch



# in ota process, erase in chunks to yield for a while in between (thus avoiding task starvation) 
patch components/app_update/esp_ota_ops.c ~/esp/GME_Binary/patches/0004_erase_ota_in_chunks_to_avoid_starvation.patch


# manage cmd 21
cd components/freemodbus/modbus
  patch -p0 < ~/esp/GME_Binary/patches/0005_manage_cmd21.patch
  
cd ../../..

# extend MQTT timeout before qos 1 publish is resent with puback received (from 1 to 2 seconds)
patch components/mqtt/esp-mqtt/mqtt_client.c ~/esp/GME_Binary/patches/0006_mqtt_extend_puback_timeout.patch

# manage read file transfer 

cd components/freemodbus/modbus
  patch -p0 < ~/esp/GME_Binary/patches/0007_add_read_filetransf.patch
...
cd ~/esp/esp-idf
patch components/esp_https_ota/src/esp_https_ota.c ~/esp/GME_Binary/patches/0008_fix_ota_2G_problem.patch
 #patch components/esp_https_ota/src/esp_https_ota.c ~/esp/GME_Binary/patches/0017_fix_ota_2G_pt1.patch

# patch to align the OTA operation of the idf version v4.3.1 to the idf version v4.0
cd ~/esp/esp-idf
patch components/esp_http_client/esp_http_client.c ~/esp/GME_Binary/patches/0009_fix_http_client_perform.patch
