 ...
 *						********************************
 *
 *      During the migration from esp-idf V4.0 to esp-idf 4.3.1
 *      we have notice that the library of HTTP management has change,
 *      mainly into the file "esp_http_client.c".
 *      Specifically, the various functions are "esp_http_client_perform" and the "http_event_handler".
 *      Currently we have decided to make a patch that modify the "esp_http_client perform" and bring back
 *      all the functionality as it worked with the idf V4.0
 *      But for the future implementations the right way will have to be to modify the handler as it explaining
 *      in the esp-idf example.
 *      This will involve changing the OTA_gme and OTA_dev and the relative http_event_handler functions.
 *
 *
 *      ---->        /home/user/esp/esp-idf/examples/protocols/esp_http_client/main/esp_http_client_example.c
 *
 * */
