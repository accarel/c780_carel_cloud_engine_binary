...
C_RES Modbus_Init(C_INT32 baud, C_SBYTE parity, C_SBYTE stopbit, C_BYTE port)
{
	 ...
	 eStatus = eMBMasterSerialInit(MB_RTU, port, baud, parity);   // ex eMBMasterInit
	 Sys__Delay(50);
...
void Modbus_Task(void)
{
	...
	while(1)
	{
		...
		if (xSentState) {
			...
			(void)xMBMasterPortEventPost(EV_MASTER_FRAME_SENT);    //EV_MASTER_FRAME_TRANSMITTED  EV_MASTER_FRAME_TRANSMIT
		}
	}
#endif
}
